    ...
    with engine.connect() as conn:
        ...
        for row in conn.execute(grade_select):
            output.append(GradeModel(
                ...
                AssignmentId=row[2],
                Grade=row[3], 
                AGH_Grade=__convert_grade_to_AGH__(row[3])
            ))
...
def getTeacherCourses(engine: Engine, teacher_id: int):
    ...
    with engine.connect() as conn:
        # Używamy dokładnie tego samego zapytania, które działa w SQL
        ...
        print(f"Executing query: {course_select}")
        for row in conn.execute(course_select):
            print(f"Found course: {row[2]} (ID: {row[1]})")
            output.append(TeacherCourseModel(Course=row[2], ID=row[1]))

    print(f"Final output: {output}")
    result = TeacherCourseListModel(CourseList=output)
